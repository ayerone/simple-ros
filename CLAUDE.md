...
**Everything a learner writes, reads, or types that refers to the ROS graph itself stays identical to real ROS 2.** Package names (`turtlesim`), node names (`/turtlesim`, `/teleop_turtle`), topic names (`/turtle1/cmd_vel`), message/service/action types (`geometry_msgs/msg/Twist`, `turtlesim_msgs/srv/Spawn`, `turtlesim_msgs/action/RotateAbsolute`), field names, and the `rclpy` API surface (`Node`, `create_publisher`, `create_subscription`, `create_service`, `create_client`, `create_timer`, `declare_parameter`, `get_parameter`, `rclpy.spin`, etc.) must all match real ROS 2 exactly. A learner's Python source files should be copy-pasteable into a real ROS 2 workspace with zero changes.
